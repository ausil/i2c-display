...
make dist
```
Creates `dist/i2c-display-0.5.3.tar.gz` with vendor directory bundled.
...
- `dist/i2c-display-0.5.3.tar.gz` — upstream source tarball (no vendor, GitHub-compatible)
- `dist/i2c-display-0.5.3-vendor.tar.bz2` — vendored dependencies (separate archive)
...
This creates:
- `rpm-build/RPMS/*/i2c-display-0.5.3-1.*.rpm` (binary RPM)
- `rpm-build/SRPMS/i2c-display-0.5.3-1.src.rpm` (source RPM)
...
This creates `../i2c-display_0.5.3-1_*.deb`
...
This creates:
- `../i2c-display_0.5.3-1.dsc`
- `../i2c-display_0.5.3-1.tar.xz`
...
2. Update changelogs:
   ...
   - `debian/changelog` — use `dch -v 0.5.3-1` or edit manually
